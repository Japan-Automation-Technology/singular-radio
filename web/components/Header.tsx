    ...
    <header className="sticky top-0 z-20 border-b border-slate-200/60 bg-white/70 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <nav className="hidden items-center gap-4 text-sm font-medium text-slate-700 md:flex">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-3 py-1 transition hover:bg-sky-50 hover:text-sky-900"
            >
              {item.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-full border border-sky-200 bg-white/80 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-sky-50 md:hidden"
                ...
                <nav className="flex flex-col gap-1 p-3 text-sm font-semibold text-slate-800">
                  {nav.map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="rounded-lg px-3 py-2 transition hover:bg-sky-50 hover:text-sky-900"
                      onClick={() => setOpen(false)}
                    >
                      {item.label}
                    </Link>
