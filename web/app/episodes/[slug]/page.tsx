            ...
            <div className="mt-3 space-y-2">
              {terms.map((term) => (
                <a
                  ...
                  className="block rounded-lg border border-sky-100/70 bg-sky-50/80 px-3 py-2 text-sm font-semibold text-sky-900 transition hover:bg-sky-100/80"
                >
                  {term.title}
                </a>
              ))}
            ...
            <ul className="mt-2 space-y-2 text-sm text-slate-700">
              {episode.transcript.slice(0, 4).map((seg) => (
                <li key={seg.id} className="flex items-center gap-2">
                  <span className="rounded-full border border-sky-100/80 bg-sky-50/80 px-2 py-1 font-mono text-xs text-sky-800">
